...
def _backfill_event_ids(news_items, event_blueprints):
    ...
    blueprint_by_id = {
        blueprint.get("event_id", ""): blueprint
        for blueprint in blueprint_payload
        if blueprint.get("event_id")
    }
    blueprint_by_url = {}
    for blueprint in blueprint_payload:
        normalized_url = _normalize_source_url(blueprint.get("source_url", ""))
        if normalized_url and blueprint.get("event_id"):
            blueprint_by_url.setdefault(normalized_url, blueprint.get("event_id", ""))

    for item in news_items or []:
        normalized_item_url = _normalize_source_url(getattr(item, "url", ""))
        exact_url_event_id = blueprint_by_url.get(normalized_item_url, "")
        if exact_url_event_id:
            item.event_id = exact_url_event_id
            continue

        current_id = getattr(item, "event_id", "") or ""
        if current_id in blueprint_by_id:
            current_score = _score_event_mapping(item, blueprint_by_id[current_id])
            competing = [
                (_score_event_mapping(item, blueprint), blueprint.get("event_id", ""))
                for blueprint in blueprint_payload
                if blueprint.get("event_id") and blueprint.get("event_id") != current_id
            ]
            best_competing_score, best_competing_id = max(competing, default=(0.0, ""))
            if current_score >= 0.20 or best_competing_score < max(0.34, current_score + 0.16):
                continue
            item.event_id = best_competing_id
            continue
...
def enforce_company_evidence(news_items, raw_results):
    """Final company-only guard, after supplementation; never regenerates rejected items."""
    from tools.company_strategy import source_level, RUMOR
    lookup = {_normalize_source_url(r.get("url", "")): r for r in raw_results or []}
    kept, warnings = [], []
    for news in news_items or []:
        result = lookup.get(_normalize_source_url(news.url))
        level = source_level(news.url)
        if "这条新闻的直接影响集中在" in news.summary:
            warnings.append(f"公司证据门禁剔除：{news.title}（旧兜底缺少事件特定的影响依据）。")
            continue
        if not result or level == "E" or RUMOR.search(str(result.get("title", "")) + " " + str(result.get("content", ""))):
            warnings.append(f"公司证据门禁剔除：{news.title}（无匹配材料或属于未确认传闻）。")
            continue
        if level != "A" and "媒体报道，尚未获得官方确认" not in news.summary:
            news.summary = news.summary.replace("【事件核心】", "【事件核心】\n媒体报道，尚未获得官方确认。", 1)
        if level in {"D", "E"}:
            news.importance = min(news.importance, 3)
        news.summary = re.sub(r"[^。！？\n]*(?:重塑行业|颠覆市场|构成直接威胁|多方消息证实)[^。！？\n]*[。！？]?", "", news.summary)
        kept.append(news)
...
def map_reduce_analysis(
    ...
    importance_requirements = (
        "importance 必须按证据分级：5 分仅用于正式重大产品/模型发布、已生效重大政策、量产交付或有明确数字支撑的行业标志事件；"
        "4 分用于有实质增量的产品升级、客户/合作、资本开支、产能或供应链节点；3 分用于有效但常规的公司动态；"
        "1 到 2 分候选原则上不进入详细新闻。不得仅因标题包含热点词就打 4 到 5 分。"
    )
    ...
    if company_tracking:
        minimum_summary_chars = 80
        detail_prompt = (
            "保留【事件核心】【深度细节/数据支撑】【行业深远影响】三段。"
            "篇幅以材料能支持的事实为限，优先于通用schema的380字建议，不设段落最低字数。"
            "核心写主体、时间、产品与动作，细节给可验证数据，影响仅写有依据的条件推断。"
            "禁止为凑字数拼接第二第三条新闻、普通背景或空泛影响。"
        )
        guidance_block += "\n原始来源账本：" + json.dumps(
            [{"url": r.get("url"), "title": r.get("title"), "content": str(r.get("content", ""))[:700]}
             for r in (raw_search_results or [])], ensure_ascii=False)
    ...
    if len(docs) <= 2:
        ...
        {guidance_block}
        {consumer_daily_rules}
        【重要度评分规则】{importance_requirements}
