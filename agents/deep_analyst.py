...
_SUMMARY_NOISY_FOLLOWUP_RE = re.compile(
    r"^(进一步看[，,:：].*)$|^(更进一步[，,:：].*)$|^(进一步(来看|来说|而言)?[，,:：].*)$|^(补充判断：围绕.*)$",
    re.IGNORECASE,
)
_SUMMARY_FOLLOWUP_TAIL_RE = re.compile(
    r"(进一步看[，,:：].*|更进一步[，,:：].*|进一步(来看|来说|而言)?[，,:：].*)",
    re.IGNORECASE,
)
_SUMMARY_SOURCE_METADATA_RE = re.compile(
    r"^(\[\.\.\.\]|\[…\]|author\s*:|published\s*:|source\s*:|原文\s*:|链接\s*:)",
    re.IGNORECASE,
)
...
def _sanitize_generated_summary(summary_text):
    ...
    for line in raw_lines:
        ...
        if skip_followup_block and not re.match(r"^【[^】]+】", cleaned):
            continue
        if re.match(r"^【[^】]+】", cleaned):
            skip_followup_block = False
        starts_followup_block = bool(_SUMMARY_NOISY_FOLLOWUP_RE.search(cleaned))
        contains_followup_tail = bool(_SUMMARY_FOLLOWUP_TAIL_RE.search(cleaned))
        cleaned = _SUMMARY_FOLLOWUP_TAIL_RE.sub("", cleaned).strip()
        cleaned = _SOURCE_LABEL_RE.sub("", cleaned).strip()
        cleaned = _GENERIC_SOURCE_SHOW_RE.sub("当前材料显示，", cleaned).strip()
        lower = cleaned.lower()
        if _SUMMARY_SOURCE_METADATA_RE.search(cleaned):
            continue
        ...
        if _SUMMARY_LOW_SIGNAL_RE.search(lower):
            continue
        if starts_followup_block or _SUMMARY_NOISY_FOLLOWUP_RE.search(cleaned):
            skip_followup_block = True
            continue
        if len(cleaned) <= 2:
            continue
        if (
...
def _build_fallback_summary(topic, blueprint, supporting_results):
    ...
    parts = [
        f"【事件核心】\n{title}。结合当前可见材料，这条新闻的主轴集中在"
        f"“{blueprint.get('event', '') or title}”所对应的业务推进、产品变化或资本市场信号上。"
        f"从主题相关性看，它与【{topic}】的技术路线、商业节奏或市场预期直接相关，"
        "需要放在同类事件中比较其真实增量，而不是只看单条标题本身。"
        "事件判断应交代清楚主体、产品或业务对象、发生动作、时间窗口、涉及市场以及可能关联的供应链环节。"
    ]
    if snippet:
        detail_lines = [_trim_text(snippet, 620)]
        ...
        if secondary_title or secondary_snippet:
            fallback_detail += f" 同时还可看到：{_trim_text(secondary_snippet or secondary_title, 300)}"
        parts.append(f"【深度细节/数据支撑】\n{fallback_detail}")
    parts.append(
...
def _expand_short_summary(summary_text, topic, blueprint, supporting_results):
    ...
    if _count_cjk_chars(summary) >= 380:
        return summary

    if _count_cjk_chars(summary) < 380:
        focus_hint = "、".join((blueprint.get("keywords", []) or [])[:4]) or blueprint.get("event", "") or topic
        supplement = (
            f"围绕“{focus_hint}”的后续观察应聚焦管理层表态、业务执行节奏、市场反馈和产业链侧印证，"
            "避免仅根据单条来源做过度外推。"
        )
        summary += f"\n{supplement}" if "行业深远影响" in summary else f"\n【行业深远影响】{supplement}"
...
def map_reduce_analysis(
    ...
    if len(docs) <= 2:
        ...
        3.4 不要在【行业深远影响】或任何段落后追加“进一步看”“更进一步”等原文摘录块；影响段只写分析判断，不粘贴来源正文。
        3.5 {event_core_requirements}
    ...
    reduce_prompt = f"""
        ...
        4.3 不要在【行业深远影响】或任何段落后追加“进一步看”“更进一步”等原文摘录块；影响段只写分析判断，不粘贴来源正文。
        4.4 {event_core_requirements}
