...
class EventDraft(BaseModel):
    ...
    event_summary: str = Field(default="", description="50到100字中文短新闻摘要，说明主体、动作、对象、关键数据或直接影响，不得编造。")
...
def _clean_event_summary_text(text):
    ...
    for raw_line in cleaned.splitlines():
        ...
        if _EVENT_SUMMARY_PATCH_RE.match(line):
            continue
        if _EVENT_SUMMARY_NOISE_RE.search(line):
            continue
...
def _trim_event_summary(text, max_chars=100):
    ...
    selected = "".join(sentences[:2]).strip() if sentences else cleaned
    if not selected:
        return ""
    if len(selected) <= max_chars:
        return selected

    clipped = selected[:max_chars].rstrip(" ，,;；。.!?！？")
...
def _build_event_summary_from_result(result, event_dict, min_chars=50, max_chars=100):
    event_text = str((event_dict or {}).get("event", "") or "").strip()
    event_has_chinese = bool(_CJK_RE.search(event_text))
    source_candidates = [
    ...
    for candidate in source_candidates:
        summary = _trim_event_summary(candidate, max_chars=max_chars)
        if summary:
            if event_has_chinese and not _CJK_RE.search(summary):
                continue
            return summary
...
def _event_summary_quality(text):
    ...
    return min(len(cleaned), 100) + min(cjk_count, 100)
...
def _merge_event_dict(existing, candidate):
    ...
    existing_summary = str(existing.get("event_summary", "") or "")
    candidate_summary = str(candidate.get("event_summary", "") or "")
    if _event_summary_quality(candidate_summary) > _event_summary_quality(existing_summary):
        merged["event_summary"] = candidate_summary
    elif existing_summary:
        merged["event_summary"] = existing_summary
...
def _rewrite_event_dicts(ai_driver, events, topic, raw_search_results=None):
    ...
    for event_dict in events:
        ...
        if matched_result and match_score >= 0.24:
            ...
            current["event_summary"] = _build_event_summary_from_result(matched_result, current)
        else:
            current["event_summary"] = _build_event_summary_from_result({}, current)
        ...
        current["event_summary"] = str(current.get("event_summary", "") or _EVENT_SUMMARY_FALLBACK)
        rewritten.append(current)
...
def generate_timeline(event_blueprints):
    ...
    for event in event_blueprints or []:
        event_dict = event.model_dump() if hasattr(event, "model_dump") else dict(event)
        event_dict["event_summary"] = str(event_dict.get("event_summary", "") or _EVENT_SUMMARY_FALLBACK)
