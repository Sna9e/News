...
_EVENT_SUMMARY_FALLBACK = "公开材料暂未披露更多细节，建议后续继续跟踪。"
_EVENT_SUMMARY_TARGET_MIN = 140
...
class EventDraft(BaseModel):
    ...
    event_summary: str = Field(default="", description="140到220字中文短新闻摘要，通常4到5句，说明主体、动作、对象、关键细节和直接影响，不得编造。")
...
def _clean_event_summary_text(text):
    ...
    if not cleaned or _EVENT_SUMMARY_PATCH_RE.match(cleaned) or _EVENT_SUMMARY_NOISE_RE.search(cleaned):
        return ""
    return cleaned
...
def _normalize_event_summary_text(text, min_chars=_EVENT_SUMMARY_TARGET_MIN, max_chars=_EVENT_SUMMARY_TARGET_MAX):
    ...
    if _is_fallback_event_summary(cleaned):
        return _EVENT_SUMMARY_FALLBACK
...
def _build_structured_chinese_summary_from_material(result, event_dict):
    material_blob = " ".join(
        str(part or "")
        for part in [
            ...
            (result or {}).get("title", ""),
            (event_dict or {}).get("event", ""),
        ]
    )
    cleaned_material = _clean_event_summary_text(material_blob)
    if not cleaned_material:
        return ""

    event_text = _strip_event_noise((event_dict or {}).get("event", ""))
    terms = _extract_material_terms(cleaned_material, event_dict)
    company_terms = [
        mapped
        for key, mapped in EVENT_TRANSLATION_HINTS.items()
        if key in cleaned_material.lower() and mapped not in terms
    ]
    subject = company_terms[0] if company_terms else (terms[0] if terms else "")
    object_terms = [term for term in terms if term != subject][:3]
    object_text = "、".join(object_terms) if object_terms else "相关产品或业务"
    action = _infer_material_action(cleaned_material)

    if _has_substantial_chinese(event_text):
        first_sentence = f"公开材料显示，{event_text}。"
    elif subject:
        first_sentence = f"公开材料显示，{subject}{action}{object_text}相关信息。"
    else:
        return ""

    source = _format_result_source(result or {}, (event_dict or {}).get("source", ""))
    date = _format_result_date(result or {}, "")
    source_part = f"{date}由{source}披露" if date and source else (f"由{source}披露" if source else "由公开来源披露")
    term_part = f"原文涉及{object_text}等对象" if object_text else "原文涉及相关业务对象"
    summary = (
        f"{first_sentence}"
        f"该线索{source_part}，{term_part}。"
        "材料没有提供足够可直接引用的中文细节，暂不能确认更多参数、客户、市场或供应链背景。"
        "因此，时间线仅记录已披露的动作本身，不补充未披露的数据或判断。"
    )
    return _normalize_event_summary_text(summary) or ""
...
def _build_event_summary_from_result(result, event_dict, min_chars=_EVENT_SUMMARY_TARGET_MIN, max_chars=_EVENT_SUMMARY_TARGET_MAX):
    ...
    for candidate in source_candidates:
        ...
        if summary:
            return summary
    structured_summary = _build_structured_chinese_summary_from_material(result or {}, event_dict or {})
...
def _ensure_event_summary(event_dict, matched_result=None):
    ...
    return selected or _EVENT_SUMMARY_FALLBACK
...
def build_event_blueprints(ai_driver, raw_search_results, topic, current_date, time_opt, history_hint="", guidance=""):
    ...
    9. 每条事件必须填写 event_summary：必须使用中文，通常为 4 到 5 个完整句子，建议 140 到 220 个中文字符；内容应覆盖主体、动作、产品/业务/政策/技术对象、原始材料明确披露的关键细节和直接影响；只能根据上方输入搜索摘要生成，不得编造，不得直接复制英文摘要，不得使用“进一步看”“更进一步”“补充判断”“值得持续关注”“后续仍需关注”“该事件具有重要意义”“相关进展值得跟踪”等空泛补句。
    """
