...
class EventDraft(BaseModel):
    ...
    event_summary: str = Field(default="", description="50到100字中文短新闻摘要，说明主体、动作、对象和关键影响，不得编造。")
...
def _normalize_event_summary_text(text, max_chars=100):
    cleaned = _trim_event_summary(text, max_chars=max_chars)
    if not cleaned:
        return ""
...
def _build_event_summary_from_result(result, event_dict, min_chars=50, max_chars=100):
    ...
    for candidate in source_candidates:
        summary = _normalize_event_summary_text(candidate, max_chars=max_chars)
        if summary:
            return summary
    return _EVENT_SUMMARY_FALLBACK
...
def _event_summary_quality(text):
    ...
    length = len(cleaned)
    if 50 <= length <= 100:
        length_score = 220 - abs(75 - length)
    elif 30 <= length < 50:
        length_score = 120 - (50 - length)
    else:
        length_score = 60
    return length_score + min(cjk_count, 100)
...
def build_event_blueprints(ai_driver, raw_search_results, topic, current_date, time_opt, history_hint="", guidance=""):
    ...
    9. 每条事件必须填写 event_summary：必须是 50 到 100 字中文短新闻摘要，说明主体、动作、产品或业务对象、关键数据或直接影响；只能基于上方输入搜索摘要，不得编造，不得直接复制英文摘要，不得出现“进一步看”“补充判断”“值得持续关注”等空泛句。
    """
