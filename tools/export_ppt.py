...
from tools.company_strategy import CONFIG as STRATEGY_CONFIG, MISSING
...
def _annotate_first_terms(prs):
    """Keep model output and source quotes intact; annotate only the rendered copy."""
    seen = set()
    terms = STRATEGY_CONFIG["glossary"]
    pattern = re.compile(r"(?<![A-Za-z])(" + "|".join(re.escape(t) for t in sorted(terms, key=len, reverse=True)) + r")(?![A-Za-z])", re.I)
    canonical = {term.lower(): term for term in terms}
    for slide in prs.slides:
        for shape in slide.shapes:
            frames = [shape.text_frame] if shape.has_text_frame else ([cell.text_frame for row in shape.table.rows for cell in row.cells] if shape.has_table else [])
            for frame in frames:
                for paragraph in frame.paragraphs:
                    for run in paragraph.runs:
                        original = run.text
                        def replace(match):
                            term = canonical[match.group().lower()]
                            if term in seen:
                                return match.group()
                            seen.add(term)
                            if re.match(r"\s*[（(][^）)]*[\u4e00-\u9fff]", original[match.end():]):
                                return match.group()
                            return f"{match.group()}（{terms[term]}）"
                        run.text = pattern.sub(replace, original)
...
def _build_news_entries(news, extraction_stats, section_warnings, compact=False, section_topic="", report_style=""):
    ...
    if event_id and not is_consumer_daily_section and report_style != "company_tracking":
    ...
    for ref in timeline_refs[:timeline_ref_limit]:
        timeline_label = "承接短新闻" if report_style == "company_tracking" else "承接时间线"
        entries.append(
            _make_entry(
                f"⏱️ {timeline_label}: [{_get(ref, 'date', '近期')}] {_shorten(_clean_display_text(_get(ref, 'event', '')), title_ref_limit)}",
...
def _add_strategy_overview_slides(prs, overview):
    def box(slide, text, x, y, w, h, size=11, bold=False, url=""):
        shape = slide.shapes.add_textbox(Inches(x), Inches(y), Inches(w), Inches(h))
        _render_entries(shape.text_frame, [_make_entry(text, size, bold, (0, 51, 102) if bold else (32, 32, 32), 0, url)])
        return shape

    def slide(title):
        page = prs.slides.add_slide(prs.slide_layouts[1] if len(prs.slide_layouts) > 1 else prs.slide_layouts[0])
        clear_placeholders(page)
        box(page, "全球科技趋势与头部企业战略动态", .5, .4, 9, .55, 22, True)
        box(page, title, .5, 1.0, 9, .4, 14, True)
        return page

    def claim(row, key, limit=30):
        return _shorten((row.get(key) or {}).get("text") or MISSING, limit)

    def table(page, headers, rows, widths, y, row_height):
        shape = page.shapes.add_table(len(rows) + 1, len(headers), Inches(.5), Inches(y), Inches(9), Inches(row_height * (len(rows) + 1)))
        for i, width in enumerate(widths):
            shape.table.columns[i].width = Inches(width)
        for ri, values in enumerate([headers] + rows):
            for ci, item in enumerate(values):
                text, url = item if isinstance(item, tuple) else (item, "")
                cell = shape.table.cell(ri, ci)
                cell.margin_left = cell.margin_right = Inches(.05)
                cell.margin_top = cell.margin_bottom = Inches(.03)
                _render_entries(cell.text_frame, [_make_entry(text, 11, ri == 0, (0, 51, 102) if ri == 0 else (32, 32, 32), 0, url)])
        return shape

    first = slide("资本投入与未来科技趋势")
    capex = {row["company"]: row for row in overview.get("capex", [])}
    rows, notes = [], []
    for company in ("Microsoft", "Google", "Amazon", "Meta"):
        row = capex.get(company, {})
        quarter = claim(row, "quarter", 26)
        rows.append([(STRATEGY_CONFIG["companies"][company]["name"].replace("（", "\n（"), (row.get("quarter") or {}).get("source_url", "")),
                     (quarter, (row.get("quarter") or {}).get("source_url", "")),
                     (claim(row, "yoy", 10), (row.get("yoy") or {}).get("source_url", "")),
                     (claim(row, "guidance", 24), (row.get("guidance") or {}).get("source_url", "")),
                     (claim(row, "revision", 12), (row.get("revision") or {}).get("source_url", "")),
                     (claim(row, "destination", 28), (row.get("destination") or {}).get("source_url", ""))])
        if row.get("period"):
            basis = str(row.get("basis", ""))
            if re.search(r"including.*finance leas|含融资租赁", basis, re.I):
                basis = "含融资租赁"
            elif re.search(r"cash|现金", basis, re.I):
                basis = "现金支出"
            elif re.search(r"property.*equipment", basis, re.I):
                basis = "购置物业设备"
            notes.append(f"{company}: 季末{row.get('period_end') or row['period']} / {_shorten(basis, 26)}")
    table(first, ["公司", "季度资本支出", "同比", "全年指引", "指引修订", "主要流向"], rows,
          [1.25, 1.65, .75, 1.65, .95, 2.75], 1.5, .48)
    box(first, "CapEx（资本支出）：现金支出与含融资租赁口径不能直接排名。数值链接至官方原文。", .5, 3.96, 9, .35, 11)
    box(first, _shorten("；".join(notes), 190) if notes else "季度和统计口径尚未核实，不填入估算。", .5, 4.3, 9, .62, 11)
    signals = {row["name"]: row for row in overview.get("trends", [])}
    for i, name in enumerate(STRATEGY_CONFIG["trends"]):
        row = signals.get(name, {})
        evidence = row.get("evidence", [])
        reason = evidence[0]["text"] if evidence else "本期证据不足，暂不评级"
        box(first, f"{row.get('arrow', '')} {name}：{_shorten(reason, 34)}", .5, 4.99 + .29 * i, 9, .29,
            11, url=evidence[0]["source_url"] if evidence else "")

    second = slide("七姐妹近期战略动作")
    companies = {row["company"]: row for row in overview.get("companies", [])}
    rows = []
    for company, config in STRATEGY_CONFIG["companies"].items():
        row = companies.get(company, {})
        action = row.get("action") or {}
        mark = "〔媒体〕" if action and action.get("source_level") != "A" else ""
        missing_action = "近期无重大新增信号" if overview.get("model_analysis_available", True) else "未完成模型分析，待复核"
        rows.append([config["name"].replace("（", "\n（"), (mark + (claim(row, "action", 37) if action else missing_action), action.get("source_url", "")),
                     (claim(row, "key_data", 22), (row.get("key_data") or {}).get("source_url", "")), row.get("direction") or MISSING,
                     (claim(row, "meaning", 35), (row.get("meaning") or {}).get("source_url", ""))])
    table(second, ["公司", "近期最重要动作", "关键数据", "技术方向", "未来意义（条件判断）"], rows,
          [1.25, 2.65, 1.35, 1.15, 2.6], 1.5, .65)
    box(second, "〔媒体〕媒体报道，尚未获得官方确认。无新增信号不代表业务停滞。", .5, 6.8, 9, .4, 11)
...
def generate_ppt(data, timeline_data, filename, model_name):
    ...
    overview = next((_get(s, "strategy_overview", None) for s in all_sections
                     if _get(s, "report_style", "") == "company_tracking" and _get(s, "strategy_overview", None)), None)
    if overview and not is_consumer_daily:
        _add_strategy_overview_slides(prs, overview)
        ...
        for t_data in timeline_data:
            ...
            for i in range(0, len(events), chunk_size):
                ...
                for idx, item in enumerate(chunk):
                    ...
                    highlight_level = str(_get(item, "highlight_level", "") or "").strip().lower()
                    if appears_later and highlight_level not in {"key", "linked"}:
                        try:
                            matched_importance = int(_get(item, "matched_news_importance", 3) or 3)
                        except (TypeError, ValueError):
                            matched_importance = 3
                        highlight_level = "key" if matched_importance >= 4 else "linked"
                    history_status = _get(item, "history_status", "")
                    prefix = "★ " if highlight_level == "key" else ("● " if highlight_level == "linked" else "")
                    ...
                    p.font.size = Pt(14)
                    p.space_after = Pt(2)
                    if highlight_level == "key":
                        p.font.bold = True
                        p.font.color.rgb = RGBColor(192, 102, 0)
                    elif highlight_level == "linked":
                        p.font.bold = True
                    ...
                    if is_company_tracking_timeline:
                        summary_text = _shorten(_format_timeline_event_summary(item), 180)
                        if summary_text:
                            p_summary = tf.add_paragraph()
                    ...
                    if appears_later:
                        ...
                        if is_company_tracking_timeline:
                            ...
                            matched_index = int(_get(item, "matched_news_index", 0) or 0)
                            detail_label = f"详细新闻 {matched_index}" if matched_index else "详细新闻"
                            p_reason.text = f"  ↳ 详见后文：{detail_label}《{matched_title}》"
                        else:
                            p_reason.text = (
                        ...
                        p_reason.font.color.rgb = (
                            RGBColor(192, 102, 0)
                            if highlight_level == "key"
                            else RGBColor(31, 78, 121)
                        )
    ...
    for section_index, section in enumerate(sections_for_output, start=1):
        ...
        if finance.get("is_public"):
            ...
            if focus_tags or extraction_stats or finance.get("data_source"):
                ...
                if extraction_stats:
                    meta_parts.append(_format_extraction_stats(extraction_stats))
                if finance.get("data_source"):
                    as_of = str(finance.get("as_of") or "").replace("T", " ")[:16]
                    source_meta = f"行情源: {finance.get('data_source')}"
                    fundamentals_source = str(finance.get("fundamentals_source") or "").strip()
                    if fundamentals_source and fundamentals_source != str(finance.get("data_source")):
                        source_meta += f" | 估值源: {fundamentals_source}"
                    if as_of:
                        source_meta += f" | 截止: {as_of}"
                    meta_parts.append(source_meta)
                meta_p.text = " | ".join(meta_parts)
                meta_p.font.size = Pt(9)
                meta_p.font.color.rgb = RGBColor(100, 100, 100)

            body_box = finance_slide.shapes.add_textbox(Inches(0.5), Inches(1.2), Inches(4.0), Inches(3.1))
            tf = body_box.text_frame
            tf.word_wrap = True
            ...
            if not data_available:
                ...
                metrics = [
                    f"▪ TTM市盈率: {_fmt_multiple(finance.get('trailing_pe'))}",
                    f"▪ 预期市盈率: {_fmt_multiple(finance.get('forward_pe'))}",
                    f"▪ 市净率: {_fmt_multiple(finance.get('price_to_book'))}  |  盈利收益率: {finance.get('earnings_yield', 'N/A')}",
                    f"▪ 估值判断: {finance.get('valuation_assessment', '估值数据不足')}",
                    f"▪ 股价判断: {finance.get('price_assessment', '价格数据不足')}",
                    f"▪ 研究观点: {finance.get('research_stance', '数据不足')}",
                    f"▪ 总市值: {finance.get('market_cap', 'N/A')}  |  52周区间: {finance.get('range_52w', 'N/A')}",
                ]
                ...
                reason_text = "；".join(finance.get("price_assessment_reasons", []) or [])
                if reason_text:
                    p_reason = tf.add_paragraph()
                    p_reason.text = f"依据: {_shorten(reason_text, 72)}"
                    p_reason.font.size = Pt(9)
                    p_reason.font.color.rgb = RGBColor(89, 89, 89)
                    p_reason.space_before = Pt(5)

                stance_reason = str(finance.get("research_stance_reason") or "").strip()
                if stance_reason:
                    p_stance = tf.add_paragraph()
                    p_stance.text = f"观点依据: {_shorten(stance_reason, 72)}"
                    p_stance.font.size = Pt(8.5)
                    p_stance.font.color.rgb = RGBColor(89, 89, 89)
                    p_stance.space_before = Pt(4)

                risk_flags = finance.get("risk_flags", []) or []
                if risk_flags:
                    p_risk = tf.add_paragraph()
                    p_risk.text = f"风险提示: {_shorten('；'.join(risk_flags), 72)}"
                    p_risk.font.size = Pt(8.5)
                    p_risk.font.color.rgb = RGBColor(192, 102, 0)
            ...
            if report_style == "company_tracking" and finance.get("strategy_enabled"):
                boxes_data = [
                    ("💰 财务与资本投入", catalysts.get("earnings", MISSING)),
                    ("🚀 技术与产品路线", catalysts.get("landmark", MISSING)),
                    ("🔗 产业链与技术传导", catalysts.get("transmission", MISSING)),
                    ("🏛️ 政策与监管", catalysts.get("policy", MISSING)),
                ]
                strategy_box = finance_slide.shapes.add_textbox(Inches(.5), Inches(6.18), Inches(9), Inches(.38))
                _render_entries(strategy_box.text_frame, [_make_entry("技术趋势与战略判断：" + _shorten(catalysts.get("style", MISSING), 65), 11, False, (32, 32, 32), 0)])

            for i, (title, content) in enumerate(boxes_data):
                x_pos = 0.5 + (i * 2.2)
                content_box = finance_slide.shapes.add_textbox(Inches(x_pos), Inches(4.45), Inches(2.1), Inches(2.1))
                ...
                p_content.text = _shorten(content, 70)
                p_content.font.size = Pt(11)
                p_content.space_before = Pt(6)

            if extraction_stats:
                stat_box = finance_slide.shapes.add_textbox(Inches(0.5), Inches(6.58), Inches(9.0), Inches(0.25))
            ...
            if section_warnings:
                warn_box = finance_slide.shapes.add_textbox(Inches(0.5), Inches(6.85), Inches(9.0), Inches(0.3))
            ...
            disclaimer_box = finance_slide.shapes.add_textbox(Inches(0.5), Inches(7.18), Inches(9.0), Inches(0.2))
            disclaimer_p = disclaimer_box.text_frame.paragraphs[0]
            disclaimer_p.text = finance.get(
                "analysis_disclaimer",
                "规则化研究观察，不构成个性化投资建议或目标价。",
            )
            disclaimer_p.font.size = Pt(8)
            disclaimer_p.font.color.rgb = RGBColor(110, 110, 110)
        ...
        for news in news_items:
            ...
            raw_title = _get(news, "title", "未命名情报")
            detail_index = int(_get(news, "detail_index", 0) or 0)
            if report_style == "company_tracking" and detail_index:
                raw_title = f"详细新闻 {detail_index}｜{raw_title}"
            title_text = _shorten(raw_title, 38 if has_chart else 56)
    ...
    if any(_get(s, "report_style", "") == "company_tracking" for s in all_sections) and not is_consumer_daily:
            ...
            for shape in page.shapes:
                ...
                for frame in frames:
                    for p in frame.paragraphs:
                        for run in p.runs:
                            run.text = re.sub(r"N/A(?:（[^）]*）)?", MISSING, run.text)
