...
_FOLLOWUP_BLOCK_START_RE = re.compile(
    r"^(进一步看[，,:：]|更进一步[，,:：]|进一步(来看|来说|而言)?[，,:：]|\[\.\.\.\]|\[…\]|author\s*:|published\s*:)",
    re.IGNORECASE,
)
...
def _append_summary_entries(entries, summary_text, body_font=13, space_after=6):
    ...
    for raw_line in str(summary_text or "").split("\n"):
        ...
        if not line:
            continue
        if skip_followup_block and not _SECTION_LINE_RE.match(line):
            continue
        if _SECTION_LINE_RE.match(line):
            skip_followup_block = False
        starts_followup_block = bool(_FOLLOWUP_BLOCK_START_RE.search(line))
        contains_followup_tail = bool(_FOLLOWUP_TAIL_RE.search(line))
        line = _FOLLOWUP_TAIL_RE.sub("", line).strip()
        if starts_followup_block or not line:
