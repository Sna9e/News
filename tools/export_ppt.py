...
_TIMELINE_SUMMARY_FALLBACK = "公开材料暂未披露更多细节。"
...
def _format_timeline_event_summary(item):
    ...
    return summary or _TIMELINE_SUMMARY_FALLBACK
            ...
            for i in range(0, len(events), chunk_size):
                ...
                for idx, item in enumerate(chunk):
                    ...
                    if is_company_tracking_timeline:
                        p_summary = tf.add_paragraph()
                        p_summary.text = _shorten(_format_timeline_event_summary(item), 118)
                        p_summary.font.size = Pt(10)
                        p_summary.font.color.rgb = RGBColor(80, 80, 80)
