...
def _validate_history_frame(hist_df):
    ...
    for column in ("Open", "High", "Low", "Close", "Volume"):
        ...
        frame[column] = frame[column].where(frame[column].map(lambda value: math.isfinite(value)))
...
def fetch_yahoo_quote_metrics(ticker_code):
    ...
    try:
        ...
        if not rows:
            return {}
        row = rows[0]
        if _normalize_ticker(row.get("symbol")) != ticker:
            return {}
        metrics = {
...
def fetch_from_yahoo_chart(ticker_code):
    ...
    try:
        ...
        if _normalize_ticker(meta.get("symbol")) != ticker:
            return None
        quote_data = {
        ...
        if any(_safe_float(quote_data.get(key)) is None for key in ("pe", "pb", "market_cap")):
            supplemental = fetch_tencent_quote_metrics(ticker)
            added = []
            # A secondary quote must never replace primary price, currency or 52w range.
            for key in ("pe", "pb", "market_cap"):
                value = _safe_float(supplemental.get(key))
                if _safe_float(quote_data.get(key)) is None and value is not None:
                    quote_data[key] = value
                    added.append(key)
            if added:
                quote_data["fundamentals_source"] = str(quote_data.get("fundamentals_source") or "yahoo_chart") + "+tencent_quote(" + ",".join(added) + ")"
...
def fetch_tencent_quote_metrics(ticker_code):
    ...
    try:
        ...
        if len(parts) < 47:
            return {}
        expected = ticker.split(".")[0].lstrip("0") if symbol.startswith(("hk", "sh", "sz")) else ticker
        returned_symbol = re.sub(r"\.(OQ|N|K)$", "", parts[2].upper()) if symbol.startswith("us") else parts[2].upper().lstrip("0")
        if returned_symbol != expected:
            return {}
        ...
        metrics = {
            ...
            "change_pct": parts[32],
            "pe": parts[39],
            # US field 46 is not reliably a price-to-book ratio. Do not guess.
            "pb": parts[46] if not symbol.startswith("us") else None,
