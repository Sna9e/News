...
from tools.company_strategy import CONFIG as STRATEGY_CONFIG, strategy_guidance
...
def get_company_query_pack(topic):
    ...
    if normalized in {"microsoft", "微软", "msft"}:
        config = STRATEGY_CONFIG["companies"]["Microsoft"]
        return {"id": "microsoft", "topic": topic, "display_name": topic,
                "aliases": config["aliases"],
                "domains": config["domains"], "keywords": config["terms"], "priority_terms": config["terms"],
                "deprioritize_terms": ["lawsuit", "stock price"],
                "summary_focus": config["summary_focus"], "high_value_signals": config["high_value_signals"],
    ...
    for pack_name, pack in MEGACAP_QUERY_PACKS.items():
        ...
        if normalized == pack_name or normalized in aliases:
            ...
            strategy = next((v for k, v in STRATEGY_CONFIG["companies"].items() if k.lower() == pack_name), {})
            payload["priority_terms"] = _dedupe(strategy.get("terms", []) + payload.get("priority_terms", []))
            payload["domains"] = _dedupe(strategy.get("domains", []) + payload["domains"])
            if strategy:
                payload["queries"] = _dedupe(payload["queries"] + ["{topic} " + " ".join(strategy["terms"][:4]) + " latest"])
            return payload
