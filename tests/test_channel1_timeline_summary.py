    ...
    return [
        {
            ...
            "event_summary": "苹果发布端侧AI计划，重点面向iPhone和Mac提供本地模型能力。公开材料显示，该计划覆盖系统体验、开发者接口和设备端推理场景。报道提到芯片算力、隐私处理和应用适配会成为落地重点。该动作将影响后续系统升级节奏，也会提高终端硬件对AI能力的配置要求。供应链和开发者需要据此调整测试与适配安排。",
...
class FakeAI:
    ...
    def analyze_structural(self, prompt, structure_class):
        if structure_class.__name__ == "EventBlueprintReport":
            ...
            assert "4 到 5 个完整句子" in prompt
            assert "140 到 220" in prompt
                ...
                events=[
                    {
                        ...
                        "source": "Apple Newsroom",
                        "event": "Apple发布端侧AI计划",
                        "event_summary": "苹果发布端侧AI计划，面向iPhone和Mac提供本地模型能力。输入材料显示，该计划会覆盖系统体验、开发者接口和设备端推理场景。相关内容提到芯片算力、隐私处理和应用适配是落地重点。该动作会影响系统升级节奏，也会提高终端硬件对AI能力的配置要求。开发者需要围绕新接口调整测试与适配安排。",
                        "source_url": "https://www.apple.com/newsroom/apple-ai",
