...
## 2026-09-07：频道一资本投入、科技趋势与七姐妹战略增量优化

- [x] 阅读现有搜索、Jina/直连/摘要兜底、公司配置、事件主档、详细新闻、金融补链与 PPT 链路；不改目录结构、搜索供应商接口、模板或走势图函数。
- [x] 新增 `tools/company_strategy.json`：七姐妹技术方向、官方域名、A-E来源级别、非官方用户内容子域、术语字典和采集上限；保留原十个公司主题及顺序，追加 Microsoft。
- [x] 新增轻量 `tools/company_strategy.py`，复用 `search_web()` 和 `fetch_single_url_with_fallback()`；四家 Hyperscaler 官方财报独立回溯150天，近期战略复用原公司搜索结果，最多一次跨公司结构化调用。
- [x] CapEx字段保留来源URL、原文连续摘录、季度结束日、统计口径；校验原文金额/单位/期间、公司对应与全文获取状态，不接受摘要估算或累计值冒充单季。
- [x] 趋势门禁：D/E不能独立支撑，官网中的播客/论坛用户内容不算官方公告，转载同一通讯社不重复计数；缺证据不生成持平箭头。
- [x] 频道一详细新闻加入来源账本、材料优先于字数的Prompt与最终证据门禁；拒绝传闻、过度确定表述与旧固定影响兜底，不重新补回已拒绝新闻。
- [x] 七姐妹金融页改为财务/资本投入、产品路线、技术传导、政策和条件化战略判断；其他公司原金融催化逻辑保留。
- [x] 修复腾讯美股公司名称字段被当作PB、日内高低价被当作52周区间、补字段覆盖Yahoo主行情的问题；校验返回代码与非有限数值。股价/涨跌幅/原走势图保留。
- [x] PPT封面之后增加恰好两页跨公司总览，保留原公司时间线和专题页；专业术语首次出现自动中文注释，无法取得的量化字段显示“暂无可靠数据”。
- [x] 新增11项测试；18个测试脚本回归通过；Streamlit AppTest五频道加载无异常；语法检查通过。
- [x] 完成真实Exa/Jina/直连与Yahoo/Tencent测试：31份材料含7份官方财报/IR材料，AAPL/MSFT各23个交易日。生成真实接口诊断PPT与DEMO满载PPT，完成图片检查及画布溢出检查。
- [ ] 本机仍未配置可用 `OPENROUTER_API_KEY`。真实模型摘要、CapEx语义提取、最终中文战略成稿尚未端到端验收，不能将DEMO视作真实研究结论。
- [ ] 有模型密钥后复跑 `python tests/run_strategy_live.py`，重点人工核对财报表格列、现金/融资租赁口径、全年指引期间、转载独立性与语义因果；IR活动索引页不一定包含完整财务指标。
- [ ] PB与Forward PE公开接口仍不稳定，保持缺失，不接新供应商、不猜测、不绕过访问控制。

## 2026-09-04：永久域名扩展与金融分析增强

- [x] 将用户指定 URL 归一化为 9 个永久域名，并同时写入内置硬屏蔽与本地永久名单：`bitrss.com`、`dev.to`、`vocus.cc`、`jethrojeff.com`、`36kr.com`、`thevergetoday.pages.dev`、`custommapposter.com`、`villadaba.com`、`xix.ai`。
- [x] 内置名单升级为版本 5、36 条规则；保持供应商请求前排除和返回结果本地复检双层门禁。
- [x] 保持 `generate_pro_kline_chart()` 和现有图表样式不变，新增 Yahoo Quote → Tencent Quote 的估值字段轻量补充；历史行情源与估值字段源分别记录。
- [x] 金融 payload 新增 TTM/预期 PE、PB、盈利收益率、5/20 日均线、5/20 日涨跌、52 周位置、估值判断、股价判断、研究观点、判断依据和风险标记。
- [x] 删除 PPT 中基于固定 4.2% 假设的股权风险溢价展示，改为可复核的盈利收益率；研究观点限制为积极/中性/谨慎观察，不生成个性化买卖指令或目标价。
- [x] 增加金融 PPT stub 自动检查，确认新增字段、观点依据、风险提示和免责声明实际进入最终 PPT，图表仍按原 `chart_path` 嵌入。
- [x] 完成 AAPL 真实公开行情烟测：Yahoo 日线与原图成功，腾讯报价补充 TTM PE，来源字段分离；完成 PPT 图片渲染与无溢出检查。
- [x] `python -m compileall -q .` 通过；17 个 `tests/test_*.py` 脚本、128 个测试函数全部通过。
- [ ] 后续如需正式目标价，应单独设计带行业可比、盈利预测、现金流和假设版本的估值模型，不在当前规则化观察中直接增加。
