...
from tools.search_engine import filter_china_results, merge_sites_text, safe_run_async_crawler, search_web
...
def build_lookup_maps(raw_results):
    ...
    for item in raw_results or []:
        ...
        snippet_lookup[url] = item.get("content", "")
...
if not st.session_state.report_ready:
    ...
    if report_warnings:
        st.warning("本次结果中存在抓取兜底专题：部分长新闻并非完整原文级分析，请结合顶部抓取概况谨慎解读细节。")
